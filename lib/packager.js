...
function Packager(options) {
  ...
  this.entries = this.assembler.entries;
  this.shared = 'shared';
...
// in preamble files.
Packager.prototype._mergeConcatOptions = function(concatOptions, legacyModules) {
  var globals = legacyModules.filter(function(mod) {
    return !mod.exports;
  });

  var sharedToAppend = globals.filter(function(mod) {
    return !mod.prepend;
  }).map(byPath);

  var sharedToPrepend = globals.filter(function(mod) {
    return mod.prepend;
  }).map(byPath);

  var sharedOptions = {
    prepend: ['__packager__/loader.js', '__packager__/vendor-prefix.js'].concat(sharedToPrepend),
    append: ['__packager__/vendor-suffix.js'].concat(sharedToAppend)
  };
...
Packager.prototype._prepTypes = function() {
  var types = [
    ...
    },
    {
      name: 'vendor',
      type: 'vendor',
      output: this.outputPaths.vendor
    },
    {
      name: 'testSupport',
      type: 'testSupport',
      output: this.outputPaths.testSupport
    }
  ];
...
Packager.prototype.createTreesFromAppImports = function(options) {
  var otherAssets = options.other.map(function(legacyMeta) {
    return funnel(legacyMeta.src, {
      include: [legacyMeta.file],
      destDir: legacyMeta.dest
    });
  });

  var testStyles = options.testStyles.map(function(legacyPath) {
    return funnel(path.dirname(legacyPath), {
      include: [path.basename(legacyPath)],
      destDir: path.dirname(getOutputPathFor(options.types, 'testSupport', 'css')),
      allowEmpty: true
    });
  });

  var vendorStyles = options.vendorStyles.map(function(legacyPath) {
    return funnel(path.dirname(legacyPath), {
      include: [path.basename(legacyPath)],
      destDir: path.dirname(getOutputPathFor(options.types, 'vendor', 'css')),
      allowEmpty: true
    });
  });
...
Packager.prototype.package = function() {
  ...
  this.concatOptions = this._mergeConcatOptions(this.concatOptions, this.assembler.legacyFilesToAppend);
  ...
  this.linkedTree = new Linker(orderedNodes, {
    ...
    legacyFilesToAppend: this.assembler.legacyFilesToAppend.concat(this.assembler.legacyTestFilesToAppend),
    entries: this.entries
  });

  var legacy = this.createTreesFromAppImports({
    types: types,
    vendorStyles: this.assembler.vendorStaticStyles,
    testStyles: this.assembler.vendorTestStaticStyles,
    other: this.assembler.otherAssetPaths
  });
  ...
  return defaultStrategy(this.assembler, {
    trees: {
      ...
      vendorStyles: legacy.vendorStyles,
      testStyles: legacy.testStyles,
      other: legacy.other
    },
    types: types,
...
function byPath(mod) {
  return mod.path.replace(/(bower_components|vendor)/, 'legacy');
}
