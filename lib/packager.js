...
Packager.prototype.package = function() {
  ...
  this.builtTree = mergeTrees([this.linkedTree].concat(
    ...
    this.assembled.treesByType('test-support'),
    this.assembled.treesByType('test-loader')
  ));

  var defaultStrategy = require('./strategies/default');
  var defaultBuild = defaultStrategy(this.builtTree,
    this.assembler.name,
    this.outputPaths,
    this.entries
  );

  return funnel(mergeTrees([defaultBuild].concat(
    this.assembled.treesByType('index'),
    this.assembled.treesByType('testem'),
    this.assembled.treesByType('test-loader')
  )), {
    destDir: '/default-build'
  });
