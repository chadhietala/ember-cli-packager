...
function getOutputPathFor(types, searchType, outputType, extension) {
  return types.filter(function(type) {
    return type.type === searchType;
  })[0].output[outputType][extension];
}
...
module.exports = function(assembler, options) {
  ...
  var testSupportStyles = concat(mergeTrees(legacyTestSupportSyles), {
    inputFiles: ['**/*.css'],
    allowNone: true,
    outputFile: getOutputPathFor(types, 'main', 'testSupport', 'css')
  });

  var vendorStyles = concat(mergeTrees(legacyCSS), {
    inputFiles: ['**/*.css'],
    allowNone: true,
    outputFile: getOutputPathFor(types, 'main', 'vendor', 'css')
  });
